...
import './ParticleEffect.css'; // Make sure to import the CSS file
...
//0: 'black',    // Wall
  1: 'white',    // Path
  2: 'red',      // rotate180
  3: 'purple',   // addRotate45
  4: '#ea82e5',  // subRotate45 - Pink
  5: '#FF5C00',  // addEdge - Orange
  6: '#46bfee',  // swap - Light blue
  start: 'yellow',
  goal: 'green',
  player: 'blue'
}; 
...
const generateMaze = (nodeDetails, connectivity_sparsity = 0.2, maze_seed = "Maze",  
  ...
  while (true) {
    ...
    //const connectivity_sparsity = 0.2;
    //const maze_seed = "Maze";
    //const operation_node_seed = null; //"here_and_there";
    if (fullyConnected) {
...
/* 
// Helper function to check if the graph is fully connected using BFS
...
/* 
const removeEdgesOnlyCrop = (adjMatrix, removalProb, seed = null, x1, y1, x2, y2) => {
  const rng = seed ? seedrandom(seed) : Math.random;
  const edgeList = [];
  const ROWS = y2 - y1 + 1;
  const COLS = x2 - x1 + 1;

  // Collect all edges within the crop area
  for (let row = 0; row < ROWS; row++) {
    for (let col = 0; col < COLS; col++) {
      const currentIdx = row * COLS + col;
      for (let neighbor of getNeighbors([row, col])) {
  ...
  for (let [currentIdx, neighborIdx] of edgeList) {
    if (rng() < removalProb) {
      ...
      // Check if the graph is still fully connected
      if (!isGraphConnectedCrop(adjMatrix, ROWS, COLS)) {
        adjMatrix[currentIdx][neighborIdx] = 1;
        adjMatrix[neighborIdx][currentIdx] = 1;
      }
    }
...
const cropAndAdjustMaze = (maze, adjMatrix, nodeDetails, [x1, y1, x2, y2], ROWS, COLS) => {
  ...
  const croppedAdjMatrix = adjMatrix.map((row, rowIndex) => 
    row.map((value, colIndex) => {
      const rowPos = Math.floor(rowIndex / COLS);
      const colPos = rowIndex % COLS;
      const neighborRowPos = Math.floor(colIndex / COLS);
      const neighborColPos = colIndex % COLS;
      
      if (
        (rowPos >= y1 && rowPos <= y2 && colPos >= x1 && colPos <= x2) &&
        (neighborRowPos >= y1 && neighborRowPos <= y2 && neighborColPos >= x1 && neighborColPos <= x2)
      ) {
        return value;
      }
      return 0;
  ...
  console.log("The adj matrix", croppedAdjMatrix);
  return { croppedMaze: updatedCroppedMaze, croppedAdjMatrix, nodeDetails };
};  */
...
/* 
const ControlPanel = ({ settings, updateSettings }) => {
  ...
  const handleMazeCropChange = (index, event, ROWS = 7,
    COLS = 7) => {
  ...
  return (
    <div style={styles.container}>
      <h3 style={styles.header}>Control Panel</h3>
      ...
      <Section title="Maze">
        ...
        <InputField
          ...
          type="number"
          value={settings.ROWS}
          onChange={(e) => handleInputChange('ROWS', e)}
        />
         <InputField
          label="Columns"
          type="number"
          value={settings.COLS}
          onChange={(e) => handleInputChange('COLS', e)}
        />
        <InputField
          label="Maze Seed"
          type="text"
        ...
        <div style={styles.inputGroup}>
          <label style={styles.label}>Maze Crop:</label>
          {[0, 1, 2, 3].map((index) => (
            <input
              key={index}
              type="number"
...
const styles = {
  container: {
    ...
    width: '350px',
    overflowY: 'auto',
    maxHeight: 'calc(100vh - 380px)', // Adjust the height to fit within the viewport
  },
  header: {
    marginBottom: '20px',
    textAlign: 'center',
  ...
  label: {
    marginBottom: '5px',
    marginRight: '20px',
    width: '100%', // Make input width 100% to ensure consistency
  },
  input: {
    padding: '5px',
    fontSize: '1em',
    borderRadius: '4px',
    border: '1px solid #ccc',
    width: '130%', // Make input width 100% to ensure consistency
...
/*   const handlePictureChange = (index, field, value) => {
  ...
  const handleDeleteElement = (index) => {
    const updatedElements = picture.elements.filter((_, i) => i !== index);
    updatePicture({ elements: updatedElements });
  }; */
...
/* 
const InteractiveMaze = ({ mazeData, onUpdateMazeData, nodeDetails }) => {
  ...
  const handleDeactivateAll = () => {
    saveToHistory({ ...mazeData }); // Save current state before change
    const newMaze = mazeData.maze.map((row) =>
      row.map(() => 0)
    );
    const newAdjMatrix = mazeData.adjMatrix.map((row) =>
      row.map(() => 0)
    );
    onUpdateMazeData({ ...mazeData, maze: newMaze, adjMatrix: newAdjMatrix });
    setNodes((prevNodes) =>
      prevNodes.map((node) => ({ ...node, type: 0 }))
    );
    setEdges([]);
  };
  ...
  const getColorForNode = (node) => {
    if (node.type === 's') {
      ...
      return nodeDetails[node.type]?.color || 'blue';
    }
  };
  ...
  return (
    <div>
      <button onClick={() => setIsAddEdgeMode((prev) => !prev)}>
        {isAddEdgeMode ? 'Disable Add Edge' : 'Enable Add Edge'}
      </button>
      <button onClick={() => setIsDeleteEdgeMode((prev) => !prev)}>
...
/*   const updateSettings = (newSettings) => {
    setSettings((prevSettings) => {
      ...
      // Ensure manual changes are applied
      if (newSettings.manual_changes) {
        const updatedMazeData = applyManualChanges(updatedMazeData, newSettings.manual_changes);
        setMazeData(updatedMazeData);
      }
  ...
  useEffect(() => {
    const generateAndSetMaze = (details) => {
      ...
      if (settings.mazeCrop) {
        ...
        // Ensure operation nodes are placed after cropping
        Object.keys(adjNodeDetails.operation).forEach((key) => {
          const operation = adjNodeDetails.operation[key];
        ...
        // Update maze with names from nodeDetails
        const updatedMaze = updateMazeWithNodeNames(croppedMaze, adjNodeDetails, true);

        setMazeData({ maze: updatedMaze, adjMatrix: croppedAdjMatrix });
        setNodeDetails(adjNodeDetails); // Set the updated node details

        setPlayerPos(adjNodeDetails.start.positions[0]);
        initializeProgress(adjNodeDetails.start.positions[0], adjNodeDetails.goal.positions, normDist); // Initialize progress
      } else {
...
/* 
  const applyManualChanges = (mazeData, manualChanges = []) => {
    manualChanges.forEach((change) => {
      if (change.type === 'node') {
        const [row, col] = change.position;
        if (change.action === 'toggle') {
    ...
    manualChanges.forEach((change) => {
      if (change.type === 'node') {
        ...
        if (change.from && change.to) {
          const fromIndex = change.from[0] * maze[0].length + change.from[1];
          const toIndex = change.to[0] * maze[0].length + change.to[1];
  
          if (
            fromIndex >= 0 && fromIndex < adjMatrix.length &&
            toIndex >= 0 && toIndex < adjMatrix[0].length
          ) {
            if (change.action === 'add') {
          ...
          console.error(`Invalid edge change: from or to position is missing`);
        }
